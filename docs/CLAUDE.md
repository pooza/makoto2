...
4. **ステージング検証（省略不可）** — 専用 CT (`bydo`) にデプロイし、投稿・応答・スケジューラの動作を目視確認する。モロヘイヤでは検証を省略したリリースが本番障害（起動クラッシュ・ネイティブ拡張欠け）を出した実績があるため、緊急ホットフィックス以外で省略しない
   ...
   ⚠ **0.1.0 でのみ省略した（2026-07-31）。****検証先の環境がまだ存在しない**ため（`bydo` はレシピ未適用・`pooza/chubo2#98`）。同じ理由で**手順 9 のデプロイも実施していない**。0.1.0 は「main にタグを打つところまで」で、**稼働はしていない**。
   **これは環境が無いことによる一度きりの例外で、前例にしない。**0.2.0 以降は `bydo` が整うまでリリースしない。
