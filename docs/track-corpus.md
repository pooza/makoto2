...
⚠⚠ **ライブ用は悉皆ではない。**実在するソロライブ（Song Party♪ vol.8・2019-11-02）のセットリスト 35 曲と照合したところ、⚠ **ライブ用に存在するのは 26 曲（74%）だけだった**（→ [birthday-live.md](birthday-live.md)）。

- **2 曲はゲストの持ち歌**で、上の定義では**原理的に拾えない**（⚠ **実際のライブにはゲストコーナーがある**）
- ⚠ **残る 7 曲は収集の取りこぼし** — iTunes に無いか、アルバム経由の走査から漏れている
