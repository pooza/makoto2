module Makoto
  ...
  class PostingJob
    ...
    def initialize(name:, timetable:, source:, tolerance: nil, visibility: nil)
      ...
      # ⚠ **覚えるのは「どこまで進んだか」の 1 点**（#126 → `claim`）。
      @claimed = nil
    ...
    # 🔴 **直前の枠だけを覚えると、A(枠 N) → B(枠 N+1) → 遅れた A(枠 N) で N をもう一度通す。**
    #
    # ⚠ **覚えるのは「どこまで進んだか」の 1 点で、そこ以前を全部断る**（#126）。
    # ⚠⚠ **直近いくつかを覚える形にしない** — 🔴 **停滞に上限が無い**（`HTTP` の timeout は
    # ソケット単位で、**相手がデータを送り続ける限り壁時計は止まらない**）ので、
    # **いくつ覚えても「それより長く停滞した tick」で同じ穴が開く**（Codex の指摘・PR #135）。
    #
    # ⚠ **落とした枠を後から拾い直さない**のは意図どおり（→ docs/CLAUDE.md
    # 「投稿の欠落は詰めない」）。⚠⚠ **枠は前にしか進まない**ので、**進んだ位置より
    # 前の枠は「もう終わった枠」しかない。**
    def claim(slot)
      return nil unless slot
      @claim_mutex.synchronize do
        return nil if @claimed && slot <= @claimed
        @claimed = slot
        return slot
      end
    end
    ...
    def post(text, slot)
      ...
      logger.error(post: @name, slot: format_slot(slot), post_length: post_length(text), error: e)
      # 🔴 **枠が落ちたことを Sentry へ**（#28）。⚠ **ライブ当日の 8 時間に静かに壊れると、
      # 気づくのは終わった後になる。**
      report_error(e, post: @name)
      record(:failure, slot)
    ...
    # ⚠⚠ **失敗の行を書くための数え方で、失敗の扱いを潰さない**（Codex の P2）。🔴 **壊れた UTF-8 は
    # 正規表現で `ArgumentError` になり、rescue の中から元の失敗・`report_error`・`record` を飛ばす。**
    def post_length(text)
      return PostBudget.length(text)
    rescue
      return nil
    end
