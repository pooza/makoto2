module Makoto
  ...
  # | 20 | 26〜🔴 **30.3** 秒 | 🔴 **`MAX_SCALE` だが、実測の max では tolerance（30 秒）を超える**（#201 の 4.） |
  ...
  #
  # 🔴 **`MAX_SCALE` の 20 は実測に追い越されている**（2026-09-23・#201 の 4.・Codex の P2）。
  # ⚠⚠ **7 回目のリハーサルの 1 本目が見かけ 15.171 秒 ＝ 実時間 1.517 秒**で、**scale 20 なら
  # 見かけ 30.3 秒** — ⚠ **`/scheduler/tolerance` の 30 秒をわずかに超える。**
  # ⚠⚠ **いま使っているのは `scale 10` なので実害は出ていない**が、🔴 **上限として置いてある値が
  # 「安全な上限」ではなくなっている** — ⚠ **下げるかはオーナー判断**（**15 なら見かけ 22.8 秒**）。
  #
  # ## ⚠⚠ 早送りで取れないもの
  #
