module Makoto
  ...
  #
  # ⚠⚠ **抽選で出る曲の約 3 分の 1（35.3%）は歌の無い曲**（⚠ **`tv_size` は歌のある
  # 曲の短縮版なので数えない** → `Song::KIND_TYPES`・#315）なので、**1 つの type だと「口ずさむ」の
  ...
  class SongSource
    ...
    # 🔴 **その枠の通し番号。**⚠⚠ **日付だけだと同じ日の枠が全部同じ前置きになる**
    # （⚠ **1 日の本数は `/song/timetable` 次第** — 🔴 **「2 本」と書いていたが #292 で
    # 3 本になっていた**。2026-09-19 に直した ＝ #315）。
    #
