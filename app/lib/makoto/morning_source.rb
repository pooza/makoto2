module Makoto
  ...
  # 落ちる。**🔴 **旧 237 件では 5 月の季節の原稿が 1 件しかなかった**ので、**段 4 が
  # 毎日勝つ形だと 31 日とも同じ挨拶**にもなった（⚠ **#225 で 12 か月すべて 16 本以上に
  # 揃えたので、いまの実データでは起きない** — 🔴 **設計の動機としては生きている**ので
  # 残す。2026-09-19 に時制を直した ＝ #285。⚠⚠ **本数は `bin/makoto morning slot`**）。
  #
  ...
  # | | |
  # | --- | --- |
  # | **季節の日** | ⚠ **月の日数 ÷ 季節の原稿の件数**で散らした日（**件数が 1 件なら月に 1 日だけ**） |
  ...
  # - ⚠ **季節の原稿の割合は「その月に何件あるか」で決まる。**⚠⚠ **月ごとの本数は
  #   `bin/makoto morning slot`**（🔴 **旧 237 件では 82 / 237 で、12 月 25 件・5 月 1 件と
  #   偏っていた** — **#225 で底を 16 本に揃えた**。→ makoto-legacy.md）
  #
  # ## 🔴 並びは周回ごとに組み替える（#223）
  #
  ...
  class MorningSource
    ...
    def call(time = nil)
      ...
      # 🔴 **連結する前に UTF-8 へ寄せる**（#280 → `Text`）。⚠⚠ **`Sequel` / SQLite が
    ...
    # 側の話 → docs/makoto-legacy.md「5 月を埋める」＝ #225 で済んだ）。
