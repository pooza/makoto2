module Makoto
  ...
  class LiveProgram
    ...
    #
    # ⚠ **`credit_text` を通すのは正規化のため** — **NFKC ＋ 空白除去**で
    # `宮本 佳那子` と `宮本佳那子` が寄る。⚠⚠ **括弧の中は落ちる**ので、
    # **`グロッサムX2(CV:宮本佳那子)` は `グロッサムX2` として扱われる ＝ 名義が出る。**
    # ⚠ **2026-08-20 のオーナー判断**（別作品の役名義で、隠すと「誰の曲か」が消える）
    # **をそのまま残している。**
    #
    ...
    def own_credit?(value)
      ...
      text = CureApiService.credit_text(value)
