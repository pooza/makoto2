module Makoto
  ...
  class TrackImporter
    ...
    # ⚠ **JSON を手で直さずこの表で当てる**のは、🔴 **`from` の歯止め**（一致した
    # ときだけ正すので、上流が直った日に `unknown` として警告に出る ＝ その行は
    # もう消せる）と、⚠⚠ **`reason` の記録**（なぜ外れていると判断したか）のため。
    # ⚠ **「分類の段がリポジトリに無いので収集で戻る」と書いていたが、#294 で
    # `seed/track_kind.py` として入っており成立しない**（2026-09-19 に直した ＝ #315）。
    ...
    # 🔴 **投稿先がメンションと読む形**（#283）。⚠ **投稿の側で上流の `escape_sigils` が崩す**ので
