...
TOPICS = {
  '食べ物・飲み物': [
    ...
    # +2026-09-06（#225 の 4 束目で足した原稿の語彙）
    # ⚠ どれも 2 文字以上の名詞そのもので、日常語に埋もれる断片ではない。
    #
    # ⚠⚠ 「卵」は入れない（Codex の P2・2026-09-06）。**「医者の卵」「卵形の顔」に当たる。**
    #    ⚠ 助詞で絞っても「医者の卵が」が残るので、絞る方向では解けない。
    #    🔴 該当の原稿は本文を「朝ごはんの卵を割ったら」に書き換えて 'ごはん' で当てた
    #    — **本文で言い換えられるなら、リストを広げない。**
    'お米', '試食', 'サンドイッチ', '常備菜',
    # +2026-09-06（#225 の 5 束目）。⚠ こちらも 3 文字以上の名詞そのもの。
    #
    # ⚠⚠ 「ジャム」は入れない（Codex の P2・2026-09-06）。🔴 **「ジャムセッション」は
    #    歌・仕事の語**で、⚠ **助詞まで含めても `ジャムのセッション` / `ジャムを録音`
    #    が残る**（**分けて書ける**）。⚠⚠ **食べ物は「低く出るのが安全側」の指標**なので、
    #    **迷う語は落とす。**🔴 該当の原稿は本文に「パン」を出して当てた。
    '目玉焼き', '焼きそば', '麦茶', '漬物',
  ],
  '歌・仕事': [
  ...
  '天気・季節': [
    ...
    '気温', '水たまり',
    # ⚠⚠ 「風がやん」は入れない（Codex の P2・2026-09-06）。**「和風がやんわり」に当たる。**
    #    🔴 「風が」＋ 動詞の形は上に 5 つあるが、どれも同じ穴を持っている — 増やさない。
    #    ⚠ 該当の原稿は本文を「風の音がやんだ瞬間」に書き換えて '風の音' で当てた。
  ],
  '髪・服・おしゃれ': [
