...
TOPICS = {
  '食べ物・飲み物': [
    ...
    # ⚠⚠ 「新米」は「まだまだ新米のスタッフ」の意味でも使う（Codex の P2）。
    '玉ねぎ', 'たまねぎ', 'たけのこ', 'オムライス', '新米を', '新米が', '味噌汁', 'みそ汁',
    'りんご',
    '唐揚げ', 'たまご焼き', '卵焼き', 'かぼちゃ', 'だしを', 'だし汁', '出汁', '昆布', '佃煮',
    '焼き魚', '煮物', '蕎麦', 'そば湯', '塩を', 'お塩', '果物', 'スープ', 'たこを', 'ゆでだこ',
    # ⚠ 「皮を剥」は「指の皮を剥いて」に当たるので入れない（Codex の P2）
    'トースト', '飴', 'レモン', '豆腐', 'わかめ', '炊け', '茹で', '火加減', 'ひとつまみ',
  ],
  '歌・仕事': [
    ...
    #   「喉」も「喉が渇いて」に当たるので絞る。
    # ⚠ 「本番」は「受験本番」「試合本番」に当たるので入れない（Codex の P2）
    'マイク', '楽屋', '拍手', '発声', '声出し', '収録', 'スタジオ', '楽譜', '振り付け',
  ...
  '天気・季節': [
    ...
    '虹', '霧', '風が吹', '風がやわ', '風が強', '風がつめた', '風が気持ち', '風の向き',
    '風の音', '風の強', 'そよ風', '向かい風', '曇り', '陽が', '日差し', '金木犀', '落ち葉',
    '新緑', '雲', '気温', '水たまり',
  ],
  '髪・服・おしゃれ': [
